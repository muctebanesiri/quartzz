...
> [!danger]- خطاهای احتمالی
> اگر دستور بالا برای شما کار نکرد، احتمالاً یکی از مشکلات زیر در حال رخ دادن است:
> - **آدرس فایل رفرنس غلط است.** بهتر است اول، همهٔ `Extra Arguments` را پاک کنید و ببینید که در حالت ساده خروجی می‌گیرد یا نه. اگر گرفت که حتماً مشکل از همان دستورهای اضافه است. اگر باز هم نگرفت، مشکل جای دیگری است.
> - **آدرس Pandoc به PATH ویندوز اضافه نشده است.** این را با اجرای دستور `pandoc -v` در CMD تست کنید. اگر خروجی دستور، ورژن `pandoc` بود، مشکلی نیست. اما اگر ارور داد، بدانید که مشکل از PATH است.
> - **در آدرس پوشه‌ها اسپِیس وجود دارد.** اگر در پوشهٔ فایل رفرنس فاصله‌ای وجود داشته باشد (مثل `New folder`)، باید آن را در بخش `Extra Arguments` تنظیمات `pandoc` به شکل `New%20folder` بنویسید. یا اینکه فایل رفرنس را به پوشه‌ای انتقال دهید که اسم آن فاصله‌ای وجود نداشته باشد.
